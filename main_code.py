...
#GLOBAL VARIABLES
interval = 0.5                                                 # The approx. duration before measureing the position of the cubesat    
image_location = "townview"                                           # Location of the image taken for saving it as a part of the image file name
...
def take_photo():
    ...
    while True:
        ...
        if (((round(accelx, accuracy) >= X_THRESHOLD)  and (round(accely, accuracy) >= Y_THRESHOLD)  and (round(accelz, accuracy) >= Z_THRESHOLD) )):
